...
#include <QLabel>
#include <QDebug>
Chatpop::Chatpop(QWidget *parent) : QWidget(parent)
{
...
void Chatpop::setText(QString text, QString time, QSize allSize, Chatpop::User_Type userType)
{
    m_msg = text;
    m_userType = userType;
    m_time = time;
    ...
    m_curTime = QDateTime::fromString(time,"yyyy/MM/dd hh:mm:ss ddd").toString("MM/dd hh:mm");
...
void Chatpop::resize_size()
{
    adjustSize();
}


void Chatpop::paintEvent(QPaintEvent *event)
{
    Q_UNUSED(event);
    ...
    }  else if(m_userType == User_Time) { // 时间
        QPen penText;
...
QSize Chatpop::fontRect(QString str)
{
    m_msg = str;
    int minHei = 30;
    int iconWH = 40;
...
void Chatpop::setIconpath(QString path,User_Type who)
{
    if(who==User_Me)
    {
        m_rightPixmap=QPixmap(path);
    }else
    {
        m_leftPixmap=QPixmap(path);
    }
}

QSize Chatpop::getRealString(QString src)
{
    QFontMetricsF fm(this->font());
    m_lineHeight = fm.lineSpacing();//行高+字高

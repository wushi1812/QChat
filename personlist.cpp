...
void personList::mousePressEvent(QMouseEvent *event)
{
    ...
    if(event->button()==Qt::LeftButton && currentItem!=NULL && currentItem==groupMap.value(currentItem))//如果点击的左键并且是点击的是组
    {
        if(isHideMap.value(currentItem))                                  //如果先前是隐藏，则显示
        {
            ...
            currentItem->setIcon(QIcon(":/img/icon/open.png"));
        }
        else                                                             //否则，先前是显示，则隐藏
        {
            foreach(QListWidgetItem* subItem, groupMap.keys(currentItem))//遍历组的对应的项（包括自身和好友）
             ...
             currentItem->setIcon(QIcon(":/img/icon/hide.png"));
        }
    }
...
{
    currentItem = this->itemAt(mapFromGlobal(QCursor::pos()));//鼠标位置的Item，不管右键左键都获取
    if(event->button()==Qt::LeftButton && currentItem!=NULL && currentItem!=groupMap.value(currentItem))//如果点击的左键并且是点击的是成员
    {
        QString id=friendidMap.value(currentItem);
        qDebug()<<"open:"+id;
...
{
    QListWidgetItem *newItem=new QListWidgetItem(QIcon(":/img/icon/hide.png"),"未命名");    //创建一个Item
    ...
    QStringList idlist=groupidMap.values();
    QList<int> intList;
    foreach(QString str, idlist) {
    ...
    qDebug()<<"add:"<<gid;
    groupidMap.insert(newItem,QString::number(gid));
    isHideMap.insert(newItem,true);  //设置该组隐藏状态
    groupNameEdit->raise();
    groupNameEdit->setText(tr("")); //设置默认内容
...
void personList::slotDelGroup()
{
    ...
    QString gid=groupidMap.value(currentItem);
    qDebug()<<"dele"<<gid;
    emit deleGroup(gid,groupname);
...
void personList::slotAddBuddy()
{
    ...
    friendidMap.insert(newItem,id);   //key为好友,value为id
...
void personList::slotRenameEditFshed()
{
    ...
    if(groupname!=NULL)      //如果重命名编辑框不为空
    {
        currentItem->setText(groupname);  //更新组名
        qDebug()<<"gname:"+groupname;
        groupNameEdit->setText("");
        groupNameEdit->hide();  //隐藏重命名编辑框
        emit addGroup(gid,groupname);
    }
